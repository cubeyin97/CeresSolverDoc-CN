# General Unconstrained Minimization 通用无约束最小化

CeresSolver除了能够求解非线性最小二乘问题，还可以求解无约束的优化问题，求解过程仅依赖目标函数的值和梯度。在这一章节我们将会了解这一过程。

## Rosenbrock 方程

现在考虑最小化注明的[Rosenbrock方程](https://en.wikipedia.org/wiki/Rosenbrock_function)。

最简单的方法是定义一个模板函数对象来计算目标函数并使用Ceres Solver的自动求导来计算导数。

首先，我们使用`AutoDiffCostFunction`来构建名为`FirstOrderFunction`的接口的实例对象。这个对象用于计算目标函数的数值以及导数（如果需要）。这一过程和定义非线性最小二乘问题如出一辙。
...
最小化它是一个简单的事情，构造一个`GradientProblem` 对象并对其调用 `Solve()`。
...
执行此代码，使用有限内存[BFGS](http://en.wikipedia.org/wiki/Broyden%E2%80%93Fletcher%E2%80%93Goldfarb%E2%80%93Shanno_algorithm)算法解决问题。
...
Final   x: 1 y: 1
```
如果你由于某种原因无法使用自动求导（例如因为需要调用外部库），那么你可以使用数值求导。函数对象的定义如下：
...
```
最后，如果你宁愿手动计算导数（比如因为参数向量的大小太大而无法自动微分）。你应该定义一个`FirstOrderFunction`的实例，它是非线性最小二乘问题的`CostFunction` 的替代形式。
